...
var bbMemo = {
  memos: "https://memos.lixiangqian.com/",
  limit: "15",
  creatorId: "1",
  domId: "#bber",
}
...
function updateHTMl(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (tagArr) {
      memosTag = tagArr
        .map(function (tag) {
          ...
          return '<span class="tag-span"># ' + tagText + "</span>"
        })
        .join("")
    } else {
      memosTag = '<span class="tag-span"># 日常</span>'
    }
    ...
    if (data[i].resourceList && data[i].resourceList.length > 0) {
      ...
      if (resUrl) {
        bbContREG += 'class="datasource">' + resUrl
      }
    }
    result += `
...
function loadArtalk(memo_id) {
  ...
  if (commentDiv.classList.contains("hidden")) {
    ...
    const artalk = new Artalk({
      ...
      pageKey: "/m/" + memo_id,
      pageTitle: "",
      server: "https://at.lixiangqian.com/",
      site: "Artalk",
      darkMode: "auto",
    })
  } else {
